...
def get_args_parser():
    ...
    """
      模型结构可选参数
    : with_box_refine 是否在Deformable DETR中使用 box_refine
    : dilation 在backbone的最后一个卷积层是否扩张
    : position_embedding 位置编码的类型：sin函数/可学习参数
    : position_embedding_scale 位置编码的周期
    : num_feature_levels 特征图的尺度数
    """
    # Bool: 是否在Deformable DETR中使用 box_refine
    parser.add_argument('--with_box_refine', default=False, action='store_true')
    # 在backbone的最后一个卷积层是否扩张
    ...
    : enc_layers 编码器层数
    : dec_layers 解码器层数
    : dim_feedforward fc层输出维度
    : hidden_dim 序列中每个元素的编码长度
    : dropout 
    : nheads
    : num_queries 解码器的query数量
    : enc_n_points 编码器参考点的采样数量
    : dec_n_points 解码器参考点的采样数量
    : no_aux_loss 禁用解码器每一层的辅助loss,默认为True禁用,设置False来使用
    """
    ...
    # 禁用解码器每一层的辅助loss, 默认为使用，设置True来禁用
    parser.add_argument('--no_aux_loss', dest='aux_loss', action='store_false',
                        help="Disables auxiliary decoding losses (loss at each layer)")

    """
      匹配器参数
    : set_cost_class 类别损失的系数
    : set_cost_bbox box位置损失的系数
    : set_cost_giou giou损失的系数
    """
    ...
    : mask_loss_coef
    : dice_loss_coef
    : focal_alpha focal loss的负样本衰减系数
    """
    ...
    : resume
    : start_epoch
    : eval 默认为False,设置True来评估
    : viz 默认为False, 设置True来可视化bonding box
    : eval_every
    : num_workers
    : cache_mode 是否在内存中缓存图片
    """
    ...
    : dataset 数据集文件名
    : data_root 数据集根目录
    : backbone 骨干网络的名称，用来从torchvision中调用骨干网络
    : PREV_INTRODUCED_CLS
    : CUR_INTRODUCED_CLS
    : top_unk
    : unmatched_boxes
    : featdim 序列中每个元素的特征维度
    : pretrain 预训练模型的名称
    : train_set 训练日志数据
    : test_set 测试日志数据
    : NC_branch novelty classification branch,默认False
    : nc_loss_coef novelty classification branch损失系数
    : nc_epoch 
    : invalid_cls_logits
    : num_classes 分类类别
    : bbox_thresh 锚框的置信度，大于该置信度回归为锚框
    """
...
#---------------------------------------------------Data Loading-------------------------------------------------------
    ...
    # 匹配函数, 如果匹配到kewords中的任何一个element, 返回true
...
#-------------------------------------------------Train Process---------------------------------------------------------
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # 每train n=args.eval_every 个epcoh, evaluate模型
